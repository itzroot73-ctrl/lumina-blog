  ...
  const infoData: Record<string, React.ReactNode> = {
    publisher: (
      <button onClick={onAuthorClick} className="flex items-center gap-2.5 group">
        <Avatar className="h-8 w-8 border border-white/10 group-hover:border-[#00f0ff]/30 transition-colors">
        ...
        <div className="text-left">
          ...
          <p className="text-[10px] text-white/30">@{authorUsername}</p>
        </div>
      </button>
    ),
    date: (
    ...
    readTime: (
      <p className="text-sm font-semibold text-white">
        {readingTime} <span className="text-white/40 font-normal">min</span>
      </p>
    ),
    category: (
      <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold uppercase tracking-wider bg-[#00f0ff]/10 text-[#00f0ff] border border-[#00f0ff]/20">
        {category}
      </span>
    ),
    views: (
      <p className="text-sm font-semibold text-white">
        {views.toLocaleString()} <span className="text-white/40 font-normal">reads</span>
      </p>
    ),
    engagement: (
      <p className="text-sm font-semibold text-white">
        {likesCount} <span className="text-white/40 font-normal">likes</span>
        <span className="text-white/20 mx-1.5">/</span>
        {commentsCount} <span className="text-white/40 font-normal">comments</span>
      </p>
    ),
  };
    ...
    >
      ...
      >
        <h3 className="text-xs font-bold uppercase tracking-[0.3em] text-white/30 flex items-center gap-2">
          <span className="w-6 h-px bg-[#00f0ff]/40" />
          Story Info
        </h3>
      </motion.div>
        ...
        {gridItems.map((item, index) => (
          ...
          >
            <div className="flex items-center gap-1.5 mb-2">
              <item.icon className="w-3 h-3 text-[#00f0ff]/50" />
